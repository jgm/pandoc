...
# The following depends on the exact wording of 'Using `pandoc`', 'Description', 'Authors' in the section heading of MANUAL.txt. Modify it accordingly if those are changed.
README.md: MANUAL.txt README.md.in
	echo "<!-- DO NOT EDIT THIS FILE, which is automatically generated from MANUAL.txt README.md.in -->" > $@
	z=$$(grep -n 'Using `pandoc`' $< | cut -f1 -d:); z=`expr $$z - 1`; y=$$(grep -n 'Description' $< | cut -f1 -d:); y=`expr $$z - $$y - 1`; perl -pe 's/<!-- Description section from the MANUAL.txt will be automatically pasted here -->/`head -'$$z' $< | tail -'$$y'`/e' README.md.in | sed -e 's/\]\[[^]]*\]/\]/g' -e 's/<!-- Authors section from the MANUAL.txt will be automatically pasted here -->//g' >> $@

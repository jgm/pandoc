...
lineBlockLine = try $ do
  char '|'
  let emptyLine    = newline >> return ""
      nonEmptyLine = do
        char ' '
        white <- many (spaceChar >> return '\160')  -- non-breakable space
...
data ParserState = ParserState
    { stateOptions         :: ReaderOptions, -- ^ User options
      ...
      stateHardBreaks      :: Bool,          -- ^ True if all newlines should be treated as hard breaks

...
{- |
   Module      : Text.Pandoc.Writers.Obsidian
   Copyright   : Copyright (C) 2025 Steph Ango
   License     : GNU GPL, version 2 or above

   Maintainer  : Steph Ango
   Stability   : alpha
   Portability : portable

Conversion of 'Pandoc' documents to Obsidian-flavored Markdown.
-}
module Text.Pandoc.Writers.Obsidian (
  writeObsidian,
  writeCommonMark,
...
writeObsidian :: PandocMonad m => WriterOptions -> Pandoc -> m Text
writeObsidian opts document =
  evalMD (pandocToMarkdown opts{
             writerWrapText = if isEnabled Ext_hard_line_breaks opts
                              then WrapNone
                              else writerWrapText opts }
             document) def{ envVariant = Obsidian } def

...
flushOuter = do
  outer <- gets stOuter
  -- we will add non-empty container inlines, or content inlines
  when (inlineLength outer > 0) $ addReady outer
  clearStOuter
  where inlineLength :: Inline -> Int
        inlineLength = maybe 1 length . maybeGetInlines
...
inlineListToRST lst = mapM inlineToRST (prepareInlines lst) >>= return . hcat
  where prepareInlines = removeSpaceAfterDisplayMath . insertBS . emergeCollapse

...
`WithDefaultPartials` and `WithPartials` are Monad wrappers.
Wrapping these around an instance of `PandocMonad` gives
different instances of `TemplateMonad`, with different
search behaviors when retrieving partials.
...
`getTemplate` seeks a template locally,
 or via HTTP if the template has an absolute URL,
 falling back to the data files if not found.

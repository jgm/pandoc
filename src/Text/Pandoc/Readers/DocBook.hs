...
parseBlock (Elem e) =
  case qName (elName e) of
        ...
        "para"  -> parseMixedWithRole para e
        "simpara"  -> parseMixedWithRole para e
        "ackno"  -> parseMixedWithRole para e
        ...
        "bibliodiv" ->
          case filterChild (named "title") e of
            Just _  -> sect 1
            Nothing -> return mempty
        "biblioentry" -> parseMixedWithRole para e
        "bibliomisc" -> parseMixedWithRole para e
        "bibliomixed" -> parseMixedWithRole para e
...
parseMixedWithRole :: PandocMonad m => (Inlines -> Blocks) -> Element -> DB m Blocks
parseMixedWithRole container e = do
  parsedContent <- parseMixed container (elContent e)
  case getRoleAttr e of
    []  -> return parsedContent
    role -> return $ divWith ("", [], role) parsedContent

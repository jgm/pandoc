...
{- |
   Module      : Text.Pandoc.Readers.Obsidian
...
Conversion of Obsidian-flavored Markdown to 'Pandoc' document.
-}
module Text.Pandoc.Readers.Obsidian (
  readObsidian,
  yamlToMeta,
...
readObsidian :: (PandocMonad m, ToSources a)
             => ReaderOptions -- ^ Reader options
             -> a             -- ^ Input
             -> m Pandoc
readObsidian opts s = do
...
block = do
  ...
  let addIdToBlock (Header lvl attr ils) = Header lvl (addBlockId attr st) ils
      addIdToBlock (Para ils) = Para ils -- TODO: need to wrap in Div
      addIdToBlock x = x
